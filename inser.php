...
switch ($op){
  case 'grava':
         ...
         //Inicio da rotina de upload de arquivo
          $maxfilesize = 500000;
          $uploader = new XoopsMediaUploader($local_folder, include_once XOOPS_ROOT_PATH.'/modules/rw_banner/class/mimetypes.inc.php', $maxfilesize);
         ...
         $form['grafico'] = XOOPS_URL.'/images/banners/rw_banner/'.$file_name;
         }
         ...
         if ($_POST['post'] == _MD_BTN_OP1){
           if($banner->grava(2))
             redirect_header(XOOPS_URL.'/modules/rw_banner/index.php',1,_MD_MSG8);
           else
             redirect_header(XOOPS_URL.'/modules/rw_banner/index.php',1,_MD_MSG10);
         }elseif ($_POST['post'] == _MD_BTN_OP2){
           if($banner->edita())
             redirect_header(XOOPS_URL.'/modules/rw_banner/index.php',1,_MD_MSG2);
           else
             redirect_header(XOOPS_URL.'/modules/rw_banner/index.php',1,_MD_MSG11);
         }
         break;
  case 'editar':
         ...
         monta_form(_MD_BTN_OP2);
         include_once(XOOPS_ROOT_PATH."/footer.php");
         break;
  default:
         include_once(XOOPS_ROOT_PATH."/header.php");
         monta_form(_MD_BTN_OP1);
         include_once(XOOPS_ROOT_PATH."/footer.php");
         break;
}
...
function monta_form($value){
  ...
  $categ = ($value == _MD_BTN_OP1)?0:$form['categoria'];
  $title = ($value == _MD_BTN_OP1)?_MD_VALUE_BTN1:_MD_VALUE_BTN3;
  ...
  $label = new XoopsFormElementTray(_MD_TITLE24,'');
  ...
  <script>
    function checa(){
      if (document.getElementById(\'form[maxexibe]\').disabled == false){
        ...
        document.getElementById(\'form[maxexibe]\').value = "'._MD_BTN_OP3.'";
      }else{
  ...
  ';
  $ilimitado = new XoopsFormCheckBox('',_MD_BTN_OP3);
  $ilimitado->setExtra('onClick="javascript:checa();" '.$check);
  $ilimitado->addOption(1,_MD_BTN_OP3);
  if ($form['maxclick'] == 0){
    $exibe1 = _MD_BANNER_EXIBREST;
    $check1 = 'checked';
    $disa1 = 'disabled';
  }else{
  ...
  $label1 = new XoopsFormElementTray(_MD_TITLE500,'');
  ...
  <script>
    function checa1(){
      if (document.getElementById(\'form[maxclick]\').disabled == false){
        ...
        document.getElementById(\'form[maxclick]\').value = "'._MD_BTN_OP3.'";
      }else{
  ...
  ';
  $ilimitado1 = new XoopsFormCheckBox('',_MD_BTN_OP3);
  $ilimitado1->setExtra('onClick="javascript:checa1();" '.$check1);
  $ilimitado1->addOption(1,_MD_BTN_OP3);
  if ($form['periodo'] == 0){
    $exibe2 = _MD_BANNER_EXIBREST;
    $check2 = 'checked';
    $disa2 = 'disabled';
  }else{
    $exibe2 = $form['periodo'];
  ...
  $label2 = new XoopsFormElementTray(_MD_TITLE5001,'');
  ...
  <script>
    function checa2(){
      if (document.getElementById(\'form[periodo]\').disabled == false){
        ...
        document.getElementById(\'form[periodo]\').value = "'._MD_BTN_OP3.'";
      }else{
  ...
  ';
  $ilimitado2 = new XoopsFormCheckBox('',_MD_BTN_OP3);
  $ilimitado2->setExtra('onClick="javascript:checa2();" '.$check2);
  $ilimitado2->addOption(1,_MD_BTN_OP3);
  $imagem = new XoopsFormText(_MD_TITLE25, "form[grafico]", 45, 255, $form['grafico']);
  $js = '
  ...
  $max_size = 5000000;
  $file_box = new XoopsFormFile(_MD_TITLE51_ED, "banner", $max_size);
  $file_box->setExtra('size ="45" onChange="vai();"') ;
  $file_box->setDescription($file_name);
  $link = new XoopsFormText(_MD_TITLE26, "form[url]", 45, 255, $form['url']);
  echo '
  <script>
    function checar(){
  ...
  $obs = new XoopsFormTextArea(_MD_TITLE5000,'form[obs]', $form['obs']);
  $obs->setDescription(_MD_TITLE5000_DESC);
  $banner_form->addElement($user_selbox);
  $banner_form->addElement($categ_selbox);
  if (in_array('maxexibe',$arr_perm) || $value == _MD_BTN_OP1){
  ...
  if (in_array('maxclick',$arr_perm) || $value == _MD_BTN_OP1){
  ...
  if (in_array('periodo',$arr_perm) || $value == _MD_BTN_OP1){
  ...
  if (in_array('grafico',$arr_perm) || $value == _MD_BTN_OP1){
  ...
  if (in_array('url',$arr_perm) || $value == _MD_BTN_OP1){
