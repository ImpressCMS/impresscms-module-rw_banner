...
function rwbanner_adminMenu ($currentoption = 0, $breadcrumb = '')
{
	...
	echo "<li id='" . $tblColors[1] . "'><a href=\"myblocksadmin.php\"\"><span>" . _ADM_MENU_TITLE2 . "</span></a></li>";
//	echo "<li id='" . $tblColors[2] . "'><a href=\"javascript:;\" onClick=\"javascript:window.open('inserecateg.php','cadastrar','width=450,height=250,toolbar=no');\"><span>" . _ADM_VALUE_BTN5 . "</span></a></li>";
	echo "<li id='" . $tblColors[2] . "'><a href=\"insercateg.php\"><span>" . _ADM_VALUE_BTN5 . "</span></a></li>";
//	echo "<li id='" . $tblColors[3] . "'><a href=\"javascript:;\" onClick=\"javascript:window.open('insere.php','cadastrar','width=450,height=310,toolbar=no');\"><span>" . _ADM_VALUE_BTN1 . "</span></a></li>";
	echo "<li id='" . $tblColors[3] . "'><a href=\"inser.php\"><span>" . _ADM_VALUE_BTN1 . "</span></a></li>";
	echo "<li id='" . $tblColors[4] . "'><a href=\"javascript:;\" onClick=\"window.open('".XOOPS_URL."/modules/system/admin.php?fct=users','cadastrar');\"><span>" . _ADM_VALUE_BTN8 . "</span></a></li>";
	echo "<li id='" . $tblColors[5] . "'><a href=\"insertag.php\"><span>" . _ADM_VALUE_BTN12 . "</span></a></li>";
	echo "</ul></div>";
}
...
function EmailStats($bid){
  ...
  $row = $xoopsDB->fetchArray($result);
    ...
    if ( $row['maxexib'] == 0 ) {
        $left = _MD_BANNER_EXIBREST;
        $row['maxexib'] = _MD_BANNER_EXIBREST;
    } else {
        $left = $row['maxexib']-$row['exibicoes'];
    }
    if ( $row['maxclick'] == 0 ) {
        $left_clicks = _MD_BANNER_EXIBREST;
        $row['maxclick'] = _MD_BANNER_EXIBREST;
    } else {
        $left_clicks = $row['maxclick']-$row['clicks'];
    }
    if ( $row['periodo'] == 0 ) {
        $left_periodo = _MD_BANNER_EXIBREST;
        $row['periodo'] = _MD_BANNER_EXIBREST;
    } else {
        $left_periodo = converte(somaData($row['data'],$row['periodo']),'BR',0);
    }
    $row['data'] = converte($row['data'],'BR',false);
    $fecha = escreveData(date("Y-n-d"));
    $subject = _MD_SUBJECT_EMAILSTATS." ".$xoopsConfig['sitename'];
    $message = _MD_BODY1_EMAILSTATS." ". $xoopsConfig['sitename']." :\n\n\n"._MD_BODY2_EMAILSTATS." $name\n"
    ._MD_BODY3_EMAILSTATS." $bid\n"
    ._MD_BODY4_EMAILSTATS." ".$row['grafico']."\n"
    ._MD_BODY5_EMAILSTATS." ".$row['url']."\n\n"
    ._MD_BODY6_EMAILSTATS." ".$row['maxexib']."\n"
    ._MD_BODY7_EMAILSTATS." ".$row['exibicoes']."\n"
    ._MD_BODY8_EMAILSTATS." $left\n"
    ._MD_BODY11_EMAILSTATS." ".$row['maxclick']."\n"
    ._MD_BODY9_EMAILSTATS." ".$row['clicks']."\n"
    ._MD_BODY10_EMAILSTATS." $percent%\n"
    ._MD_BODY12_EMAILSTATS." $left_clicks\n"
    ._MD_BODY13_EMAILSTATS." ".$row['data']."\n"
    ._MD_BODY14_EMAILSTATS." ".$row['periodo'].' '._MD_BODY17_EMAILSTATS."\n"
    ._MD_BODY15_EMAILSTATS." $left_periodo\n\n\n"
    ._MD_BODY16_EMAILSTATS." $fecha";
...
function &rwbanner_getModuleInfo()
{
	...
	if (!isset($rwModule)) {
	    global $xoopsModule;
	    if (isset($xoopsModule) && is_object($xoopsModule) && $xoopsModule->getVar('dirname') == 'rw_banner') {
	        $rwModule =& $xoopsModule;
	    }
	    else {
	        $hModule = &xoops_gethandler('module');
	        $rwModule = $hModule->getByDirname('rw_banner');
	    }
	}
	return $rwModule;
}
