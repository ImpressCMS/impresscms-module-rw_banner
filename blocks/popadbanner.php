...
function exibe_adbanner($options){
  ...
  $block['title'] = _MI_RWBLOCK3_NAME;
  $block['lang_mb_text1'] = _MB_TEXT1;
...
function edita_adbanner($options){
  ...
  $qtde = _MB_OPTION14."&nbsp;<input type='text' name='options[]' value='".$options[3]."' onchange='javascript:options3.value = this.value;' />";
  $form .= "<br />".$qtde;
  //Será exibido novamente em quantos reloads?
  $qtde = _MB_OPTION5."&nbsp;<input type='text' name='options[]' value='".$options[4]."' onchange='javascript:options4.value = this.value;' />";
  $form .= "<br />".$qtde;
  //Cor de fundo
  $qtde = _MB_OPTION6."&nbsp;<input type='text' name='options[]' value='".$options[5]."' onchange='javascript:options5.value = this.value;' />";
  $form .= "<br />".$qtde;
  //Cor da Borda
  $qtde = _MB_OPTION7."&nbsp;<input type='text' name='options[]' value='".$options[6]."' onchange='javascript:options6.value = this.value;' />";
  $form .= "<br />".$qtde;
  //Espaço entre os banners
  $qtde = _MB_OPTION8."&nbsp;<input type='text' name='options[]' value='".$options[7]."' onchange='javascript:options7.value = this.value;' />";
  $form .= "<br />".$qtde;
  $mostrar = "<br />"._MB_OPTION9."&nbsp;<select options[8] name=\"options[8]\" onchange='javascript:options8.value = this.value;'>";
  if ($options[8] == 1)
    $mostrar .= '<option value="1" selected>'._MB_TEXT2.'</option>';
  else
    $mostrar .= '<option value="1">'._MB_TEXT2.'</option>';
  if ($options[8] == 0)
    $mostrar .= '<option value="0" selected>'._MB_TEXT3.'</option>';
  else
    $mostrar .= '<option value="0">'._MB_TEXT3.'</option>';
  $mostrar .= '</select>';
  $form .= $mostrar.'<br /><br />'._MB_OPTION10;
