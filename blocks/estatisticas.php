...
function estatisticas_banner($options){
  ...
  if ($xoopsUser){
      ...
      for($i = 0; $i <= count($arr3)-1; $i++){
        $arr3[$i]['exibrest'] = ($arr3[$i]['maxexib'] == 0)?_MB_BANNER_EXIBREST:round($arr3[$i]['maxexib']-$arr3[$i]['exibicoes']);
        ...
        $arr3[$i]['link'] = '<a class="'.$arr3[$i]['class'].'" href="javascript:void(0);" onclick="javascript: window.open(\''.XOOPS_URL.'/modules/rw_banner/admin/exibe.php?id='.$arr3[$i]['codigo'].'\',\'editar\',\'width='.($arr3[$i]['larg']+20).',height='.$arr3[$i]['alt'].',toolbar=no\');">'.$arr3[$i]['codigo'].'</a>';
        $data = $arr3[$i]['data'];
    	$periodo = $arr3[$i]['periodo'];
    	$maxdata = somaData($data,$periodo);
        if ($periodo == 0){
          $arr3[$i]['periodo'] = _MB_BANNER_EXIBREST;
        }else{
      ...
      $block['lang_msg1'] = _MB_MSG1;
      $block['lang_cod'] = _MB_CODBANNER;
      $block['lang_imp'] = _MB_IMP;
      $block['lang_rest'] = _MB_IMPRES;
      $block['lang_clic'] = _MB_CLIQUES;
      $block['lang_perc'] = _MB_PORCCLI;
      $block['lang_data'] = _MB_DATA;
      $block['lang_periodo'] = _MB_PERIODO;
      $block['lang_button1'] = _MB_BUTTON1;
      $block['lang_button2'] = _MB_BUTTON2;
  }else{
    $block['lang_msg2'] = sprintf(_MB_MSG2,$xoopsConfig['sitename'],XOOPS_URL.'/user.php',XOOPS_URL.'/register.php');
  }
  return $block;
}
?>
